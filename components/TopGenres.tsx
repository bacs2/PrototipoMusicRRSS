...
export function TopGenres({ genres }: Props) {
  ...
  return (
    <div className="rounded-2xl bg-surface-container-low p-5 space-y-4">
      ...
      {genres.length === 0 && (
        <p className="text-sm text-on-surface-variant">Sin datos de géneros todavía.</p>
      )}

      <div className="space-y-3">
        {genres.map((g, i) => (
          <div key={g.genre} className="flex items-center gap-3">
            <span className="w-5 shrink-0 text-xs font-bold text-on-surface-variant/50 tabular-nums text-right">
              {i + 1}
            </span>
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-xs text-on-surface-variant tabular-nums ml-2 shrink-0">
                  {g.count}
                </span>
              </div>
